...
reghdfe log_pub_count treated##post##is_top_aff if is_female == 0, absorb(author_id year##fieldid) vce(cluster author_id)

reghdfe log_pub_count treated##post##is_top_aff if is_female == 1, absorb(author_id year##fieldid) vce(cluster author_id)

reghdfe log_pub_count treated##post##is_top_aff if first_entry_year < 2000, absorb(author_id year##fieldid) vce(cluster author_id)
